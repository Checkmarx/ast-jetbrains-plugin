package com.checkmarx.intellij.common.window.actions.filter;

import com.checkmarx.intellij.common.resources.CxIcons;
import lombok.Getter;

import javax.swing.*;
import java.util.Set;
import java.util.function.Supplier;

/**
 * Link severity with an icon.
 */
@Getter
public enum SeverityFilter implements Filterable {
    MALICIOUS(CxIcons.Medium.MALICIOUS),
    CRITICAL(CxIcons.Medium.CRITICAL),
    HIGH(CxIcons.Medium.HIGH),
    MEDIUM(CxIcons.Medium.MEDIUM),
    LOW(CxIcons.Medium.LOW),
    INFO(CxIcons.INFO);

    public static final Set<Filterable> DEFAULT_SEVERITIES = Set.of(MALICIOUS,CRITICAL, HIGH, MEDIUM, LOW);

    private final Icon icon;

    SeverityFilter(Icon icon) {
        this.icon = icon;
    }

    public Supplier<String> tooltipSupplier() {
        return this::toString;
    }

    @Override
    public String getFilterValue() {
        return this.name();
    }

    public static SeverityFilter fromID(String id) {
        if (id.startsWith("Checkmarx.")) {
            return SeverityFilter.valueOf(id.substring(id.indexOf('.') + 1).toUpperCase());
        }
        throw new IllegalArgumentException("Invalid ID for severity");
    }
}