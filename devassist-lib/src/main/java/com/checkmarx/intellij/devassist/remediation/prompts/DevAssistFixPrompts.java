...
public final class DevAssistFixPrompts {
    ...
    public static String buildSCARemediationPrompt(String packageName, String packageVersion,
        ...
        prompt.append("---\n\n")
                ...
                .append("  - `pypi`/`setuptools`: `python -c \"import ").append(packageName).append("\"`, `pytest`, `python -m build`\n")
                .append("  - `pyproject.toml` (Poetry/uv): `python -c \"import ").append(packageName).append("\"`, `pytest`, then `uv lock` or `poetry lock` if lock present\n")
