...
class ProblemBuilderTest {
    ...
    @Test
    @DisplayName("build uses WEAK_WARNING for shell script files")
    void testBuildUsesErrorForShellScripts() throws Exception {
        ...
        // Mock createProblemDescriptor with WEAK_WARNING for shell scripts (3 LocalQuickFix for non-OSS/CONTAINERS)
        when(manager.createProblemDescriptor(eq(psiFile), any(TextRange.class), anyString(),
                eq(ProblemHighlightType.WEAK_WARNING), eq(true),
