...
public class RealtimeInspection extends LocalInspectionTool {
    ...
    public ProblemDescriptor[] checkFile(@NotNull PsiFile file, @NotNull InspectionManager manager, boolean isOnTheFly) {
        ...
        this.problemManager.addToCxOneFindings(file, scanResult.getIssues());
        return problems.toArray(new ProblemDescriptor[0]);
    }

    private boolean isRealTimeScannerActive(String filePath) {
        Optional<ScannerService<?>> scannerService = scannerFactory.findRealTimeScanner(filePath);
        return scannerService.filter(service ->
                ScannerUtils.isScannerActive(service.getConfig().getEngineName())).isPresent();
    }
    ...
    private ScanResult<?> scanFile(PsiFile file, String path) {
        ...
        if (!ScannerUtils.isScannerActive(scannerService.get().getConfig().getEngineName())) {
            this.problemManager.removeAllGutterIcons(file);
            return null;
        }
    ...
    private List<ProblemDescriptor> createProblemDescriptors(@NotNull PsiFile file, @NotNull InspectionManager manager, boolean isOnTheFly,
        ...
        this.problemManager.removeAllGutterIcons(file);
        ScanIssueProcessor processor = new ScanIssueProcessor(this.problemManager, file, manager, document, isOnTheFly);
        ...
        LOGGER.debug("REAL-TIME-SCAN: Problem descriptors created: {} for file: {}", +problems.size(), file.getName());
        return problems;
    }
}
