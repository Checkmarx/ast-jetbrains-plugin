...
public class RealtimeInspection extends LocalInspectionTool {
    ...
    public ProblemDescriptor[] checkFile(@NotNull PsiFile file, @NotNull InspectionManager manager, boolean isOnTheFly) {
        ...
        problemManager.addToCxOneFindings(file, scanResult.getIssues());

        ProblemHolderService.getInstance(file.getProject()).addProblems(file.getVirtualFile().getPath(), scanResult.getIssues());
    ...
    private List<ProblemDescriptor> createProblemDescriptors(@NotNull PsiFile file, @NotNull InspectionManager manager, boolean isOnTheFly,
        ...
        for (ScanIssue scanIssue : scanResult.getIssues()) {
            System.out.println("** Package name: " + scanIssue.getTitle());

            List<Location> locations = scanIssue.getLocations();
            if (Objects.isNull(locations) || locations.isEmpty()) {
                continue;
            }
            // Example: Line number where a problem is found (1-based, e.g., line 18)
            int problemLineNumber = locations.get(0).getLine() + 1;
            ...
            if (problemManager.isLineOutOfRange(problemLineNumber, document)
                    || scanIssue.getSeverity() == null || scanIssue.getSeverity().isBlank())
                continue;
            try {
                boolean isProblem = problemManager.isProblem(scanIssue.getSeverity().toLowerCase());
                if (isProblem) {
                    ProblemDescriptor problemDescriptor = createProblem(file, manager, scanIssue, document, problemLineNumber, isOnTheFly);
     ...
     * @param scanIssue  OssRealtimeScanPackage scan result
     ...
     */
    private ProblemDescriptor createProblem(@NotNull PsiFile file, @NotNull InspectionManager manager, ScanIssue scanIssue, Document document, int lineNumber, boolean isOnTheFly) {
        try {
            ...
            String description = problemManager.formatDescription(scanIssue);
            ProblemHighlightType problemHighlightType = problemManager.determineHighlightType(scanIssue);
