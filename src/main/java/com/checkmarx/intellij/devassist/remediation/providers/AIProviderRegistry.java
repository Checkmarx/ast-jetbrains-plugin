...
public final class AIProviderRegistry {
    ...
    /**
     * Gets the preferred provider based on user settings.
     * 
     * <p>
    ...
    public Optional<AIProvider> getPreferredProvider(@Nullable Project project) {
        String preferredId = GlobalSettingsState.getInstance().getPreferredAIProvider();

        if (AUTO_PROVIDER_ID.equals(preferredId) || preferredId == null || preferredId.isEmpty()) {
            // Auto-select: return first available provider by priority
