...
public class GlobalSettingsComponent implements SettingsComponent {
    ...
    private void installMcpAsync(String credential) {
        CompletableFuture.supplyAsync(() -> {
            try {
                ...
                // Use reflection to call McpSettingsInjector.installForCopilot if DevAssist plugin is available
                return callMcpInstall(credential);
            } catch (Exception ex) {
                return ex;
            }
    ...
    private void showWelcomeDialog(boolean mcpEnabled) {
        try {
            // Use reflection to show WelcomeDialog if DevAssist plugin is available
            callShowWelcomeDialog(mcpEnabled);
        } catch (Exception ex) {
                ...
                e -> {
                    ...
                    if (configurable != null) {
                        ...
                        ApplicationManager.getApplication().invokeLater(() -> {
                            // Try to open CxOneAssist settings if DevAssist plugin is available
                            callShowAssistSettings();
                        });
                    }
                }
        );
        ...
        logoutButton.addActionListener(e -> {
            ...
            if (userChoice == Messages.YES) {
                ...
                java.util.concurrent.CompletableFuture.runAsync(() -> {
                    try {
                        // Use reflection to call McpSettingsInjector.uninstallFromCopilot if DevAssist plugin is available
                        boolean removed = callMcpUninstall();
                        if (!removed) {
    ...
    // ========== DevAssist Integration (Optional) ==========
    // These methods use reflection to call DevAssist plugin components if available.
    // This allows plugin-core to work independently without a compile-time dependency on plugin-devassist.

    /**
     * Calls McpSettingsInjector.installForCopilot using reflection if DevAssist plugin is available.
     * @param credential The credential to install
     * @return Boolean.TRUE if MCP was modified, Boolean.FALSE if already up-to-date, or an Exception
     */
    private Object callMcpInstall(String credential) {
        try {
            Class<?> mcpClass = Class.forName("com.checkmarx.intellij.devassist.configuration.mcp.McpSettingsInjector");
            java.lang.reflect.Method method = mcpClass.getMethod("installForCopilot", String.class);
            return method.invoke(null, credential);
        } catch (ClassNotFoundException e) {
            LOGGER.debug("DevAssist plugin not available - skipping MCP install");
            return Boolean.FALSE;
        } catch (Exception e) {
            LOGGER.warn("Failed to call MCP install via reflection", e);
            return e;
        }
    }

    /**
     * Calls McpSettingsInjector.uninstallFromCopilot using reflection if DevAssist plugin is available.
     * @return true if MCP entry was removed, false otherwise
     */
    private boolean callMcpUninstall() {
        try {
            Class<?> mcpClass = Class.forName("com.checkmarx.intellij.devassist.configuration.mcp.McpSettingsInjector");
            java.lang.reflect.Method method = mcpClass.getMethod("uninstallFromCopilot");
            return (Boolean) method.invoke(null);
        } catch (ClassNotFoundException e) {
            LOGGER.debug("DevAssist plugin not available - skipping MCP uninstall");
            return false;
        } catch (Exception e) {
            LOGGER.warn("Failed to call MCP uninstall via reflection", e);
            return false;
        }
    }

    /**
     * Shows WelcomeDialog using reflection if DevAssist plugin is available.
     * @param mcpEnabled Whether MCP is enabled
     */
    private void callShowWelcomeDialog(boolean mcpEnabled) {
        try {
            Class<?> dialogClass = Class.forName("com.checkmarx.intellij.devassist.ui.WelcomeDialog");
            java.lang.reflect.Constructor<?> constructor = dialogClass.getConstructor(Project.class, boolean.class);
            Object dialog = constructor.newInstance(project, mcpEnabled);
            java.lang.reflect.Method showMethod = dialogClass.getMethod("show");
            showMethod.invoke(dialog);
        } catch (ClassNotFoundException e) {
            LOGGER.debug("DevAssist plugin not available - skipping welcome dialog");
        } catch (Exception e) {
            LOGGER.warn("Failed to show welcome dialog via reflection", e);
        }
    }

    /**
     * Shows CxOneAssist settings dialog using reflection if DevAssist plugin is available.
     */
    private void callShowAssistSettings() {
        try {
            Class<?> configurableClass = Class.forName("com.checkmarx.intellij.devassist.settings.CxOneAssistConfigurable");
            @SuppressWarnings("unchecked")
            Class<? extends Configurable> typedClass = (Class<? extends Configurable>) configurableClass;
            com.intellij.openapi.options.ShowSettingsUtil.getInstance()
                    .showSettingsDialog(project, typedClass);
        } catch (ClassNotFoundException e) {
            LOGGER.debug("DevAssist plugin not available - cannot show assist settings");
        } catch (Exception e) {
            LOGGER.warn("Failed to show assist settings via reflection", e);
        }
    }
}